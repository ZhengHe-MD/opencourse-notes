...
举一个具体例子：A 和 B 各有一组数字 $X$ 和 $Y$，他们想要计算两组数字的 sum (总和)，这时 A (B) 可以先计算 $X'$ 的 sum，然后发给 B (A)，然后 B (A) 也同样计算手上数字的 sum，并将其和 A (B) 发送的 sum 加总，得到结果。这里的 sum 就是我们说的 sketch。
...
在实践中，$X$ 可能由不断产生的样本点 $x$ 构成，我们无法一次性获取，甚至永远无法获取样本总体 $X$，这时就希望能维持当前已获得样本 $X'$ 的 sketch $C(X')$。以刚才的求和为例，我们可以维护 $X'$ 和 $Y'$ 的 running sum，这个过程就是 streaming。streaming 很常见，比如你的本地路由器就会通过计算流量 (traffic) 的 sketch 来寻找 pattern。
...
比如机器学习中最常见的 regression problems (回归问题)：先搜集一批样本点 $\{(z_i, b_i|i = 1,...,n)\}$，假设存在 $b_i = f(z_i) + noise$，我们期望能够找到尽可能接近 $f$ 的 $\tilde{f}$。而最简单的回归方法就是 linear regression (线性回归)，假设存在 $x$ 使得 $f(z) = \langle x, z \rangle$，且 noise 符合 Gaussian/Normal distribution，最优的估计就是 Least Squares (最小二乘法)：
$$
x^{LS} = arg\space min \|Zx - b\|_2^2 = (Z^TZ)^{-1}Zb
$$
其中 $b = (b1,...,b_n)^T)$，$Z = (z_1,...,z_n)^T$。如果 $Z$ 很大，即样本数量巨大，直接使用 matrix multiplication (矩阵乘法) 将消耗大量计算资源。在之后的课程中，我们会学习更快的近似计算 $(Z^TZ)^{-1}Z$ 的方法。
...
在本课中，我们主要讨论的是 discrete random variables (离散变量)，假设存在一个离散变量 $X$，其所有可能取值组成集合 $S$，那么它的 expectation (期望) 就是：$\mathbb{E}(X) = \sum_{j\in S} j \cdot \mathbb{P}(X = j)$。

### 2.1 Linearity of Expectation

不同 random variables 的 expectation 线性可加：
$$
...
> 小结：Markov's Inequality 为一个 non-negative random variable 取值超过某个常数 $\lambda$ 的概率给出上限
...
对任意 discrete random variable $X$，有 Chebyshev's Inequality 成立：
$$
...
> 小结：Chebyshev's Inequality 为任意 random variable 的取值与 expectation 的距离超过某个常数 $\lambda$ 概率给出上限。

### 2.4 Chernoff Bound

假设 $X_1,...,X_n$ 是 independent random variables，且 $X_i \in [0, 1]$，定义 $X = \sum_iX_i, \lambda > 0,$
$$
...
> 小结：Chernoff Bound 为多个 random variables 的求和距离其 expectation 的距离超过某个常数 $\lambda$ 的概率给出 exponentially decreasing (呈指数下降) 的上限。
...
一种最简单直接的方式就是维护一个精确的 counter，假设事件发生总数为 $n$，那么 counter 占用的空间大小为 $\log{n}$ bits。根据 Pigeonhole Principle，如果要精确计数，我们不可能使用比 $log\space n$ bits 更小的空间。
...
首先，我们可以用直觉来推测，如果要设计一个确定性的 (deterministic) 算法来解决上述问题，我们的空间复杂度下限是 $O(\log\log{n})$ bits。因为我们最多可以记录 $$\log{n} 个从小到大的区间，且区间大小呈指数增长，那么原本记录事件发生次数的 counter 就变成记录当前处于第几个区间的 counter，因此就能推测出空间复杂度满足 $O(\log\log{n})$。实际上，下面的 Morris 算法就能满足我们的空间复杂度下限。
...
Morris++ 的思路是：**利用 $t$ 个 failure probability 为 $\frac{1}{3}$ 的 Morris+ 副本，取结果的 medium (中位数)** 。
...
\end{cases}
$$
其中 $\mathbb{P}(|Y_i - n| > \varepsilon n) < \frac{1}{3}$。如果 medium 估计错误，那么至少有 $\frac{t}{2}$ 个 Morris+ 副本估计错误，因此：
$$
\begin{align*}
